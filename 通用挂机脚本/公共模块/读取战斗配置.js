...
var configModeArray = [
	{
		...
		think: (ctx) => {
			...
			// 如果发现需要烧的技能，则调整技能顺序并选择好默认目标
			if (skill) {
				// 制作要修炼技能的的战斗配置
				var skillObj = cga.skill.getSkill(skill.name)
				// 仅允许战斗中可以主动使用的技能，不然会出现脚本想在战斗中开制造的情况。
				if (skillObj.target != '其它') {
	...
	inputcb: (cb) => {
		...
		cga.waitForChatInput((msg, index) => {
			if (index !== null && index >= 1 && professionalArray[index - 1]) {
				...
				thisobj.finalJob = cga.job.getJob(professionalArray[index - 1].name);
