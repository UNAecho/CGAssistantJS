...
var loop = () => {
	...
	if (thisobj.finalJob.jobType == '服务系') {
		if (thisobj.finalJob.job == '医师' || thisobj.finalJob.job == '护士') {
			console.log('医师和护士可以参与保证书的转职烧声望环节')
		} else {
	...
	/**
	 * 声望第13阶段是敬畏的寂静
	 * 如果现阶段声望大于敬畏的寂静，或者离线记录声望上限大于敬畏的寂静，并且完美调教术符合预期，则退出转职保证书烧声望逻辑
	 * 
	 * 如果人物需要完美调教术，但是人物当前的状态是驯兽师，并且处于学习完美调教术的进度中（练级、晋级、烧声望等），则不转职
	 */
	if ((jobObj.reputationLv > 13 || thisobj.configObj.maxreputationlv > 13) && (!thisobj.needPerfectTrainSkill || (thisobj.needPerfectTrainSkill && (cga.findPlayerSkill('完美调教术') || jobObj.job == '驯兽师')))) {
		setTimeout(() => {
			updateConfig.update_config({ 'mainPlugin': '智能练级' })
		}, 5000)
		return
