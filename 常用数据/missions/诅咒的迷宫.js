var thisobj = {
	...
	taskStages: [
		...
		},
		{
			 ...
			 * 5、各种状态魔法（单体/强力/超强）、各种状态抵抗或各种制御魔法
			 * 6、最终BOSS，不可以跳过
			 */
			...
			workFunc: function (cb2) {
				...
				let loop = () => {
					...
					if (thisobj.data.isTeamLeader) {
						cga.walkList(thisobj.func.getPath(mapindex), () => {
							let curMapIndex = cga.GetMapIndex().index3
							if(curMapIndex == nextStageObj.bossRoomIndex){
								cga.walkList(thisobj.func.getPath(curMapIndex), () => {
	...
	taskRequirements: [//任务阶段是否完成
		...
		function () {//8.诅咒的迷宫为固定迷宫，每10层一个关卡，需战胜BOSS或满足通关条件后方可通过。进入迷宫前先组队，并进入诅咒的迷宫内部。
			let mapindex = cga.GetMapIndex().index3
			return mapindex > 24008 && mapindex < 24019
		},
		function () {//9.走迷宫，并且过5位BOSS。脚本可以在任意时刻中断并重新执行。
			let mapindex = cga.GetMapIndex().index3
			return mapindex == 24067
		},
	],
	data: {// 任务数据，可自定义，方便使用
		isTeamLeader: false,
		...
		path: {
			...
			24060: [[23, 4, '诅咒的迷宫 地下53楼'], [13, 14, '诅咒的迷宫 地下53楼'],],
			24061: [[25, 23, '诅咒的迷宫 地下54楼'], [19, 17, '诅咒的迷宫 地下54楼'], [15, 5, '诅咒的迷宫 地下54楼'],],
			24062: [[23, 4, '诅咒的迷宫 地下55楼'], [15, 11, '诅咒的迷宫 地下55楼'],],
			24063: [[15, 8, '诅咒的迷宫 地下56楼'],],
			24064: [[25, 12, '诅咒的迷宫 地下57楼'], [18, 25, '诅咒的迷宫 地下57楼'],],
			24065: [[23, 3, '诅咒的迷宫 地下58楼'], [13, 15, '诅咒的迷宫 地下58楼'],],
			24066: [[20, 15, '诅咒的迷宫 地下59楼'],],
			24067: [[8, 13, '第六个难关']],
