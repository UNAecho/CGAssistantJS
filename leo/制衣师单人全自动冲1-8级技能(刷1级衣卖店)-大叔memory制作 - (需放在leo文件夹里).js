...
require('./common').then(cga => {
	...
	// leo.log('大叔memory尝试自采自造1级旅人之服制作脚本，需要使用leo的模块启动');
	var doctorName = '大叔-医治苍生';
	var countmany = 1; //制作次数
	var skillLevel = 1; //造几级， 目前只写了造1级的
	var productName="旅人之服" ;
	var skill = cga.findPlayerSkill('制衣服');
	if(!skill){
	...
	var main = async () => {
		...
		while(cga.getItemCount('印度轻木') < 150 && cga.getItemCount('旅人之服') < 1){
			await leo.logBack()
			await leo.checkHealth(doctorName)
		...
		///////////////////////////////// 3.买布模块开始	
		leo.log('买布模块开始');	
		while(cga.getItemCount('麻布')<60 && cga.getItemCount('旅人之服') < 1){
	...
	//		await leo.goto(n=>n.teleport.yer)
		...
		while(cga.getItemCount('鹿皮')>=20 
			...
			await leo.log('材料已集齐，开始去制造一级 旅人之服')
			await leo.logBack()
			await leo.checkHealth(doctorName)
			await leo.loop(()=>{
