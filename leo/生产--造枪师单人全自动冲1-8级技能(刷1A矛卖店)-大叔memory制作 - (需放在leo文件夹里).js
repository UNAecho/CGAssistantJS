...
require('./common').then(cga => {
	...
	// leo.log('大叔memory尝试自采自造1级短矛制作脚本，需要使用leo的模块启动');
	var doctorName = '大叔-医治苍生';
	var countmany = 1; //制作次数
	var skillLevel = 1; //造几级， 目前只写了造1级的
	var productName="短矛" ;
	var skill = cga.findPlayerSkill('造枪');
	if(!skill){
	...
	var main = async () => {
		...
		while(cga.getItemCount('铜条') < 32 && cga.getItemCount('短矛') < 1){
			await leo.logBack()
		...
		while(cga.getItemCount('印度轻木') < 160 && cga.getItemCount('短矛') < 1){
			await leo.logBack()
			await leo.checkHealth(doctorName)
...
*/
		...
		while(cga.getItemCount('铜条')>=4 
			...
			await leo.log('材料已集齐，开始去制造一级1A 短矛')
			await leo.logBack()
			await leo.checkHealth(doctorName)
			await leo.loop(()=>{
			...
			await leo.log('造矛完成第' + (countmany++) + '轮')
		}
