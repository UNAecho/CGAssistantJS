            ...
            .then(() => {
                ...
                if (isTeamLeader) {
                    ...
                    if (mapInfo.name == '国民会馆' && mapInfo.x == 107 && (mapInfo.y == 51 || mapInfo.y == 52)) {
                        return leo.supply(108, 52).then(() => {
						    return leo.statistics(leo.beginTime, leo.oldXp);//打印统计信息
                        });
                    }
                    if (mapInfo.name.indexOf('雪拉威森塔５０层')!=-1){
                        return leo.encounterTeammate(protect, '雪拉威森塔５０层'); //队员遇敌
                    }
                }
                //console.log('延时3秒');
