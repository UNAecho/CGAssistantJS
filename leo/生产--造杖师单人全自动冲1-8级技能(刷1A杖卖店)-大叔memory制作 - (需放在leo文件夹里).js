...
require(process.env.CGA_DIR_PATH+'/leo').then(async (cga) => {
	...
	var productName="短杖" ;
	var skill = cga.findPlayerSkill('造杖');
	if(!skill){
	...
	var main = async () => {
		...
		while(cga.getItemCount('铜条') < 32 && cga.getItemCount('短杖') < 1){
			await leo.logBack()
		...
		while(cga.getItemCount('印度轻木') < 160 && cga.getItemCount('短杖') < 1){
			await leo.logBack()
			await leo.checkHealth(doctorName)
...
*/
		...
		while(cga.getItemCount('铜条')>=4 
			...
			await leo.log('材料已集齐，开始去制造一级1A 短杖')
			await leo.logBack()
			await leo.checkHealth(doctorName)
			await leo.loop(()=>{
			...
			await leo.log('造杖完成第' + (countmany++) + '轮')
		}
