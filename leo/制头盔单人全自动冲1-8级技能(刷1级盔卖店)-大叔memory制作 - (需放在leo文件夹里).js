...
require('./common').then(cga => {
	...
	var productName="硬皮头盔" ;
	var skill = cga.findPlayerSkill('造头盔');
	if(!skill){
		console.error('提示：没有造头盔技能！');
		return;
	}
	if(skill.lv<skillLevel){
		console.error('提示：造头盔技能等级不足，至少要'+skillLevel+'级技能！');
		return;
	}
	...
	var main = async () => {
		...
		///////////////////////////////// 3.买布模块开始	
		leo.log('买布模块开始');	
		while(cga.getItemCount('麻布')<30 && cga.getItemCount(productName) < 1){
			var count = 30 - cga.getItemCount('麻布');
			await leo.logBack()
	//		await leo.goto(n=>n.teleport.yer)
		...
		while(cga.getItemCount('鹿皮')>=20 
			...
			await leo.loop(()=>{
				...
				if (cga.getItemCount('鹿皮') < 20 || cga.getItemCount('麻布') < 2 ) {
					return leo.reject();
				}
			...
			await leo.log('造头盔完成第' + (countmany++) + '轮')
		}
